...
//=> 动态创建DOM的方式(外层容器基于createElement完成, 容器中的具体内容可以基于创建DOM完成, 也可以基于字符串拼接完成)
...
//=> 基于文档碎片(虚拟内容中开辟的一个容器)可以解决这个问题: 每当创建一个li, 我们首先把它存放到文档碎片中(千万不要放到页面中, 避免回流), 当我们把需要的元素都创建完成, 并且都添加到文档碎片中, 再统一把文档碎片放到页面中(只会引发一次回流操作)
...
document.querySelector('.productBox').innerHTML = str;//=> 只引发一次DOM回流, +=str是把原有容器中的结构都以字符串的方式获取到, 然后和新的str字符串拼接, 最后统一再插入到原有的容器中.    =str是用新的字符串替换原有的结构
//实际上, ES6字符串拼接不如操作Fragment和appendChild这种方式, 因为Fragment是直接保存的DOM对象, 而ES6中保存的是字符串, 需要先把字符串转换为DOM对象.
